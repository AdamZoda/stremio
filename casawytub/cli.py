...
def first_launch_setup():
    ...
    draw_box(
        [
            f"{C.WHITE}{C.BOLD}Bienvenue dans CasaWyTub !{C.RST}",
            f"",
    ...
    if not check_yt_dlp():
        ...
        draw_box(
            [
                ...
                f"",
                f"{C.GRAY}Puis relancez CasaWyTub.{C.RST}",
            ],
    ...
    try:
        ...
        try:
            if parse_ver(current_version) < parse_ver(latest):
                ...
                draw_box(
                    [
                        f"{C.YELLOW}{C.BOLD}Une mise à jour de CasaWyTub est disponible ! ({latest}){C.RST}",
                        f"",
...
def show_help():
    ...
    lines = [
        ...
        f"  {C.CYAN}quit{C.RST}    {C.DARK}│{C.RST}  {C.GRAY}Quitter CasaWyTub{C.RST}",
        f"",
...
def repl():
    ...
    while True:
        try:
            cmd = input(f"  {C.CYAN}{C.BOLD}casawytub{C.RST} {C.DARK}›{C.RST} ").strip()
        except (EOFError, KeyboardInterrupt):
    ...
    if len(sys.argv) > 1:
        ...
        if arg in ("--help", "-h"):
            ...
            lines = [
                ...
                f"",
                f"  {C.CYAN}casawytub{C.RST}                      {C.GRAY}Mode interactif (REPL){C.RST}",
                f"  {C.CYAN}casawytub{C.RST} {C.WHITE}<url>{C.RST}                {C.GRAY}Télécharger une vidéo (MP4){C.RST}",
                f"  {C.CYAN}casawytub{C.RST} {C.WHITE}<url>{C.RST} {C.YELLOW}--audio{C.RST}        {C.GRAY}Télécharger en audio (MP3){C.RST}",
                f"  {C.CYAN}casawytub{C.RST} {C.WHITE}<url>{C.RST} {C.YELLOW}--video{C.RST}        {C.GRAY}Télécharger en vidéo (MP4){C.RST}",
                f"",
                f"{C.DARK}Alias : -a (audio), -v (vidéo), --mp3, --mp4{C.RST}",
            ]
            draw_box(lines, title="CasaWyTub CLI — Aide", border_color=C.BLUE)
            print()
            sys.exit(0)
